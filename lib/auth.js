...
import { credentialsOf, getUserById, sanitizeUser } from "@/lib/db";
...
function signingKey(hash) {
  return createHmac("sha256", SERVER_SECRET).update(String(hash)).digest();
}

export function createToken(user) {
  const hash = credentialsOf(user);
  const payload = b64(JSON.stringify({ u: user.id, e: Date.now() + MAX_AGE * 1000 }));
  const sig = createHmac("sha256", signingKey(hash)).update(payload).digest("base64url");
  return `${payload}.${sig}`;
}
...
export function verifyToken(token, user) {
  ...
  const expected = createHmac("sha256", signingKey(hash)).update(payload).digest("base64url");
...
export async function setSession(user, { secure = false } = {}) {
